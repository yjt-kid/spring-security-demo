...
public class RedisUtil {
    ...
    public void setTokenRefresh(String token, SelfUserDetails userDetails) {
        //刷新时间
        Integer expire = validTime * 24 * 60 * 60 * 1000;
        hset(token, "id", userDetails.getId() + "", expire);
        hset(token, "username", userDetails.getUsername(), expire);
        hset(token, "password", userDetails.getPassword(), expire);
        List<UrlGrantedAuthority> list = (List<UrlGrantedAuthority>) userDetails.getAuthorities();
        String json = GsonUtil.createGson().toJson(list);
        hset(token, "authorities", json, expire);
    }
}
